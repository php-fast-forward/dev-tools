...
- when structured output is active, DevTools suppresses progress-heavy nested
  tool output where possible and forwards JSON, quiet, or no-progress flags to
  subprocesses that support them;
...
- ``--json`` and ``--pretty-json`` also suppress PHPUnit progress output
  automatically;
