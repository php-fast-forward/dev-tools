...
.. code-block:: bash
   ...
   composer docs --source=docs --target=build
...
- it calls ``docs --target build``;
- it calls ``tests --coverage build/coverage --no-progress --coverage-summary``;
- it calls ``metrics --target build/metrics --junit build/coverage/junit.xml``;
