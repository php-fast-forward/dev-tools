...
8.5 language features and tooling support.
...
- Treat PHP 8.5 as the compatibility floor for local development.
- If a consumer library tests against newer engines, keep the local minimum at
  8.5 unless the library itself is intentionally raising its baseline.
- When debugging dependency resolution, remember that the package sets
  ``config.platform.php`` to ``8.5.0``.