...
- Add a reusable structured-output foundation and expose the first machine-readable `--format=json` rollouts on `changelog:entry`, `changelog:check`, `changelog:next-version`, `changelog:promote`, and `changelog:show` for automation workflows (#33)
